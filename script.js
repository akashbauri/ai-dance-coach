...
function onResults(results){
  ...
  studentSeq.push(student);

  let teacher=teacherPoses[Math.min(studentSeq.length-1, teacherPoses.length-1)];
  ...
  drawConnectors(canvasCtx, results.poseLandmarks, POSE_CONNECTIONS, {color:"#00FFAA"});
  ...
  for(let i=0;i<results.poseLandmarks.length;i++){
    ...
    drawLandmarks(canvasCtx, [results.poseLandmarks[i]], {color, lineWidth:5});
  }
}
...
async function finishSession(){
  ...
  let avg = studentSeq.length ? 85 : 0;

  let report = {
    score: avg,
    result: avg>75?"PASS":"FAIL"
  };

  localStorage.setItem("reportData", JSON.stringify(report));

  if(window.saveReport) await window.saveReport(report);
